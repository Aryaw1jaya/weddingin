					...
					<table class="table table-bordered text-center">
						...
						<tbody>
							<?php foreach($order_detail as $od) : ?>
								<tr>
									...
									<td><h5>Rp. <?= number_format($od['price']) ?></h5></td>
								</tr>
							<?php endforeach ?>
						</tbody>
						<tfoot class="bg-success text-light">
							<tr>
								...
								<td><h5><strong>Rp. <?= number_format(array_sum(array_column($order_detail, 'subtotal'))) ?></strong></h5></td>
							</tr>
						</tfoot>
					</table>
				</div>
					...
					<form action="<?= base_url("order/update/" . $order['id']) ?>" method="post">
						...
						<div class="input-group">
							<select name="status" class="form-control">
								...
								<option value="noted" <?= $order['status'] == 'noted' ? 'selected' : '' ?>>noted</option>
